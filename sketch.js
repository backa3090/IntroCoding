...
let triggerOffset = 20; // helps remove gap
...
let chordStartX = bubbleX + 580;
let chordY = bubbleY + 40;
let chordSpacing = 70;
...
function gotSpeech() {
  ...
  if (!tempo) {
    if (spoken.includes("fast")) tempo = "fast";
    else if (spoken.includes("medium")) tempo = "medium";
    else if (spoken.includes("slow")) tempo = "slow";
  }

  if (chords.length < 4) {
    if (spoken.includes("tritone") || spoken.includes("tritone sub") || spoken.includes("tri tone sub")) {
  chords.push("Tri");
}
else if (spoken.includes("french") || spoken.includes("french augmented six") || spoken.includes("f r plus 6")) {
  chords.push("Fr+6");
}
else if (spoken.includes("five") || spoken.includes("5") || spoken.includes("five seven")) {
  chords.push("V7");
}
else if (spoken.includes("four") || spoken.includes("4") || spoken.includes("four seven")) {
  chords.push("IV7");
}
else if (spoken.includes("two") || spoken.includes("2") || spoken.includes("two seven")) {
  chords.push("ii7");
}
else if (spoken.includes("one") || spoken.includes("1") || spoken.includes("one seven")) {
  chords.push("I7");
}
  }

  if (tempo && chords.length === 4) {
    speechRec.stop();

    if (tempo === "fast") bpm = 120;
    if (tempo === "medium") bpm = 90;
    if (tempo === "slow") bpm = 60;
...
function runSequencer() {
  ...
  if (measureIndex !== lastMeasureIndex && measureIndex < 4) {
    ...
    let sound = (measureIndex < 3) ? g1[chord] : g2[chord];
...
function drawTempoUI() {
  ...
  // manually position these inside the bubble
  let positions = [
    { x: bubbleX + 470, y: bubbleY + 193 },   // FAST (bottom)
    { x: bubbleX + 470, y: bubbleY + 143},   // MEDIUM (middle)
    { x: bubbleX + 470, y: bubbleY + 97 }    // SLOW (top)
  ];
...
function drawBPMVisuals() {
  ...
  let currentImage = (beatIndex % 2 === 0) ? imgA : imgB;
